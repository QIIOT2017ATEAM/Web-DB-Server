...
//Login in slim
$app->post('/password_change', function () use ($app) {
  session_start();
  //session_destroy();
  include "db_functions.php";
  include '../signup_confirmation/connection/connect.php';
  ...
  if(isset($_POST['password_change_btn']))
  {
    $user_id = $_POST['user_id'];
    $user_password = $_POST['user_password'];
    $change_password1 = $_POST['change_password1'];
    $change_password2 = $_POST['change_password2'];
    
    if(empty($user_id) || empty($user_password) || empty($change_password1) || empty($change_password2))
    {
      //$error = "<div class='text-danger'>Please fill out the form!</div>";
      $error = "Please fill out the form!";
      echo("<script>location.replace('../password_change.php?error=".$error."');</script>");
    }
    else
    {
      //First, Serch ID
      ...
      if(isset($users[0]))
      {
        ...
        if(password_verify($user_password, $users['user_password']))
        {
          ...
          {
            $hash_password = password_hash($change_password1, PASSWORD_DEFAULT);
          ...
          {
            $error = "The passwords you want to change are different!";
            echo("<script>location.replace('../password_change.php?error=".$error."');</script>");
          }
        }
        else
...
//air-as-json start
$app->post('/air-as-json', function () use ($app) {
  include "db_functions.php";

  $macaddress = $_POST['macaddress'];

  try {
    $sth = $pdo->prepare('SELECT * FROM udoo_data WHERE macaddress = :macaddress order by num desc limit 1');
    $sth->bindValue(':macaddress',$macaddress);
    $sth->execute();
...
$app->post('/co_dynamic_chart_json', function () use ($app) {
  include "db_functions.php";
...
$app->post('/no2_dynamic_chart_json', function () use ($app) {
  include "db_functions.php";
  ...
  try {
    $sth = $pdo->prepare('SELECT *
      ...
      $app->post('/so2_dynamic_chart_json', function () use ($app) {
        include "db_functions.php";
        ...
        try {
          $sth = $pdo->prepare('SELECT *
            ...
            $app->post('/o3_dynamic_chart_json', function () use ($app) {
              include "db_functions.php";
              ...
              try {
                $sth = $pdo->prepare('SELECT *
                  ...
                  $app->post('/pm25_dynamic_chart_json', function () use ($app) {
                    include "db_functions.php";
                      ...
                      $sth = $pdo->prepare('SELECT *
                        FROM (SELECT *
                          ...
                          if ($result) {
                            ...
                            $json_array['cols'] = array(
                              array('id'=>'', 'label'=>'Time', 'type'=>'string'),
                              array('id'=>'', 'label'=>'PM2.5 Value', 'type'=>'number'));
