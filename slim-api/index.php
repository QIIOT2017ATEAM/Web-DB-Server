  ...
  $app->get('/simple/:name', function ($name) use ($app) {
    ...
    $app->get('/distance/:location/:radius', function ($location, $radius) use ($app) {
      ...
      $app->post('/saveupload', function () use ($app) {
        ...
        move_uploaded_file($_FILES['file']['tmp_name'], "/var/www/teama-iot/csv/".$new_name);

        http_response_code(200);
        $app->response->headers->set('Content-Type', 'application/json');
        return $app->response->write(json_encode(array("status"=>"success", "path"=>"/var/www/teama-iot/csv/$new_name")));
      });
